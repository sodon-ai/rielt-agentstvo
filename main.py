...
def send_reset_email(to_email, username, code):
    ...
    <p>Здравствуйте, {username}!</p>
    <p><a href="https://rielt-agentstvo.onrender.com/reset_password_form?code={code}">Сбросить пароль</a></p>
    </body></html>
    """
    ...
    for c in codes:
        if c["code"] == code:
            if datetime.fromisoformat(c["expires_at"]) > datetime.now():
                user = get_user_by_username(c["email"])
                if user:
                    update_user_by_username(user["username"], {"email_verified": True})
                codes.remove(c)
                save_data(DATA_FILES["email_verifications"], codes)
                return True
    return False
...
def add_notification(to_user, message, date, type, property_id):
    ...
    if user and user.get("email") and user.get("email_verified") and EMAIL_CONFIG["sender_email"]:
        send_notification_email(user["email"], user.get("nickname", user["name"]), "Новое уведомление", message)
...
async def index(request: Request):
    ...
    <section class="about-section"><div class="container"><h2>Риэлторское агентство</h2><p><strong>В чем суть работы риэлторского агентства?</strong> Это помощь клиенту в подборе и продаже недвижимости...</p>
    ...
    <h3>Услуги риэлторского агентства в Лянторе</h3><ul class="about-list"><li><strong>Качественный подбор недвижимости</strong></li><li><strong>Быстрая и выгодная продажа объекта</strong></li><li><strong>Юридические услуги</strong></li></ul></div></section>
...
async def buy(request: Request, search: str = "", min_price: str = "", max_price: str = "", min_area: str = "", max_area: str = "", type_filter: str = "", sort: str = ""):
    ...
    filtered = []
    for p in props:
        if search and search.lower() not in p["title"].lower() and search.lower() not in p["location"].lower():
            continue
    ...
    for p in filtered:
        ...
        cards += f'<div class="property-card"><h3>{p["title"]}</h3><p><strong>📍 Локация:</strong> {p["location"]}</p><p><strong>💰 Цена:</strong> {p["price"]} ₽</p><p><strong>📐 Площадь:</strong> {p["area"]} м²</p><p><strong>👀 Просмотров:</strong> {p.get("views", 0)}</p>{photos_html}<div class="property-actions">'
        
        if user:
            cards += f'<form action="/add_to_comparison/{p["id"]}" method="post" style="display:inline;"><button type="submit" class="compare-btn">📊 Сравнить</button></form>'
            ...
            if user.get("role") in ["client", "seller", "admin"]:
                cards += f'<form action="/buy_property/{p["id"]}" method="post" style="display:inline;"><button type="submit" class="buy-btn" onclick="return confirm(\'Купить?\')">💰 Купить</button></form>'
                cards += f'<form action="/rent_property/{p["id"]}" method="post" style="display:inline;"><button type="submit" class="rent-btn" onclick="return confirm(\'Арендовать?\')">🔑 Арендовать</button></form>'
            if user.get("role") == "admin":
                cards += f'<form action="/delete_property/{p["id"]}" method="post" style="display:inline;"><button type="submit" class="delete-btn" onclick="return confirm(\'Удалить?\')">🗑️ Удалить</button></form>'
    ...
    <section style="padding:50px 0;"><div class="container">
        <h1 style="color:var(--dark-blue);margin-bottom:30px;">🏠 Квартиры и дома</h1>
        <form method="get" class="search-bar">
            <div class="search-group"><label>🔍 Поиск</label><input type="text" name="search" placeholder="Поиск..." value="{search}"></div>
...
async def favorites(request: Request):
    ...
    for p in fav_props:
        ...
        if p.get("photos") and len(p["photos"]) > 0:
            photos_html = f'<div class="property-photos"><img src="{p["photos"][0]}" class="property-photo" onclick="openModal(this.src)"></div>'
        cards += f'<div class="property-card"><h3>{p["title"]}</h3><p><strong>💰 Цена:</strong> {p["price"]} ₽</p>{photos_html}<div class="property-actions"><form action="/remove_favorite/{p["id"]}" method="post"><button type="submit" class="delete-btn">❤️ Удалить</button></form><a href="/buy?search={p["title"]}" class="btn">🔍 Смотреть</a></div></div>'
    if not cards:
        cards = "<p>У вас пока нет избранных объявлений</p>"
    
    content = f'<section style="padding:50px 0;"><div class="container"><h1>❤️ Избранное</h1><div class="cards-grid">{cards}</div></div></section>'
...
async def compare(request: Request):
    ...
    if not compare_props:
        content = '<section style="padding:50px 0;"><div class="container"><h1>📊 Сравнение</h1><p>Нет объектов для сравнения</p><a href="/buy" class="btn">Перейти к поиску</a></div></section>'
        theme = user.get("theme", "light")
        return HTMLResponse(render_html(content, user=user, theme=theme))
    
    table = '<table style="width:100%;border-collapse:collapse;"><tr style="background:var(--light-blue);"><th style="padding:12px;">Параметр</th>'
    for p in compare_props:
        table += f'<th style="padding:12px;">{p["title"]}</th>'
    table += '</tr>'
    
    rows = [("📍 Локация", lambda x: x["location"]), ("💰 Цена", lambda x: f'{x["price"]} ₽'), ("📐 Площадь", lambda x: f'{x["area"]} м²'), ("🏗️ Этаж", lambda x: f'{x["floor"]} / {x["total_floors"]}'), ("🏠 Тип", lambda x: x["building_type"]), ("👀 Просмотров", lambda x: x.get("views", 0))]
    ...
    content = f'<section style="padding:50px 0;"><div class="container"><h1>📊 Сравнение</h1>{table}<div style="margin-top:20px;"><form action="/clear_comparison" method="post"><button type="submit" class="delete-btn" onclick="return confirm(\'Очистить?\')">🗑️ Очистить</button></form></div></div></section>'
    theme = user.get("theme", "light")
    return HTMLResponse(render_html(content, user=user, theme=theme))

@app.post("/add_to_comparison/{prop_id}")
async def add_to_comparison_route(request: Request, prop_id: int):
    user = get_user_from_session(request)
    if not user:
        return RedirectResponse("/login?error=Авторизуйтесь", 303)
    user_data = get_user_by_username(user["username"])
...
async def buy_property_route(request: Request, prop_id: int):
    user = get_user_from_session(request)
    if not user:
        return RedirectResponse("/login?error=Авторизуйтесь", 303)
...
async def rent_property_route(request: Request, prop_id: int):
    user = get_user_from_session(request)
    if not user:
        return RedirectResponse("/login?error=Авторизуйтесь", 303)
...
async def chat(request: Request):
    ...
    content = f'<section style="padding:50px 0;"><div class="container"><h1>💬 Общий чат</h1><div class="chat-messages" id="chatMessages">{html}</div>{form}{user_list}</div></section><script>function loadMessages(){{fetch("/get_messages").then(r=>r.json()).then(d=>{{document.getElementById("chatMessages").innerHTML=d.html;}});}}setInterval(loadMessages,3000);loadMessages();</script>'
    theme = user.get("theme", "light") if user else "light"
    return HTMLResponse(render_html(content, user=user, theme=theme))
...
async def private_chat(request: Request, to_user: str):
    ...
    if not msgs:
        msgs_html = '<p>Сообщений пока нет</p>'
    ...
    <section style="padding:50px 0;"><div class="container">
        ...
        <a href="/chat" class="btn" style="margin-top:20px;">← Назад</a>
    </div></section>
    <script>
        function loadMessages(){{fetch("/get_private_messages/{to_user}").then(r=>r.json()).then(d=>{{document.getElementById("chatMessages").innerHTML=d.html;}});}}
...
async def send_private_message_route(request: Request, to_user: str, message_text: str = Form(...)):
    user = get_user_from_session(request)
    if not user:
        return RedirectResponse("/login", 303)
    add_private_message(user["username"], user.get("nickname", user["name"]), to_user, message_text, datetime.now().strftime("%H:%M:%S"))
    add_notification(to_user, f"💬 Новое сообщение от {user.get('nickname', user['name'])}", datetime.now().strftime("%d.%m.%Y %H:%M"), "private_message", 0)
...
async def info(request: Request):
    ...
    trans_html = ""
    for t in trans[:20]:
        trans_html += f'<div class="transaction-item"><strong>📅 {t["date"]}</strong><p>🏠 {t["property_title"]}</p><p>💰 {t["price"]} ₽</p><p>👤 {t["seller"]} → {t["buyer"]}</p><p>📝 {t["type"]}</p></div>'
    if not trans:
        trans_html = "<p>Пока нет сделок</p>"
    ...
    if user and user.get("role") == "admin":
        ...
        for u in users_list:
            ...
            if not u.get("is_banned") and u["username"] != "artem_gaun":
                ban_button = f'<form action="/ban_user/{u["username"]}" method="post" style="display:inline;"><button type="submit" class="ban-btn" onclick="return confirm(\'Забанить?\')">🚫 Забанить</button></form>'
            elif u.get("is_banned"):
                ban_button = f'<form action="/unban_user/{u["username"]}" method="post" style="display:inline;"><button type="submit" class="unban-btn" onclick="return confirm(\'Разбанить?\')">✅ Разбанить</button></form>'
            users_html += f'<div class="user-item"><strong>👤 {u.get("nickname", u["name"])}</strong> (@{u["username"]})<br>Роль: {u["role"]}<br>Статус: {"🔴 Забанен" if u.get("is_banned") else "🟢 Активен"}<br>{ban_button}</div>'
        
        deleted = get_deleted_messages()
        del_html = ""
        for d in deleted[:30]:
            del_html += f'<div class="deleted-item"><strong>🗑️ {d["nickname"]}</strong> <small>{d["time"]}</small><p>{d["text"]}</p><div style="font-size:12px;">Удалено: {d["deleted_by"]} ({d["deleted_at"]})</div></div>'
        
        props = get_all_properties()
        total_views = sum(p.get("views", 0) for p in props)
        
        admin_html = f'''
        <div class="info-section"><h2>📈 Статистика</h2><p>Всего объявлений: {len(props)}</p><p>Всего просмотров: {total_views}</p><p>Всего пользователей: {len(users_list)}</p><p>Сделок: {len(trans)}</p></div>
        <div class="info-section"><h2>👥 Пользователи</h2>{users_html}</div>
        <div class="info-section"><h2>🗑️ Корзина</h2>{del_html if deleted else "<p>Корзина пуста</p>"}<form action="/clear_deleted_messages" method="post" style="margin-top:10px;"><button type="submit" class="delete-btn" onclick="return confirm(\'Очистить корзину?\')">🗑️ Очистить корзину</button></form></div>
        <div class="info-section"><h2>📎 Экспорт</h2><a href="/export_transactions" class="btn">📊 Выгрузить сделки в Excel</a></div>'''
    
    content = f'<section style="padding:50px 0;"><div class="container"><h1>📊 Сведения о сделках</h1><div class="info-section"><h2>📋 История сделок</h2>{trans_html}</div>{admin_html}</div></section>'
    theme = user.get("theme", "light") if user else "light"
    return HTMLResponse(render_html(content, user=user, theme=theme))

@app.get("/export_transactions")
async def export_transactions_route(request: Request):
    user = get_user_from_session(request)
    if not user or user.get("role") != "admin":
        raise HTTPException(403, "Только для администратора")
    trans = get_all_transactions()
...
async def register_post(request: Request, name: str = Form(...), username: str = Form(...), nickname: str = Form(...), email: str = Form(...), password: str = Form(...), role: str = Form(...)):
    ...
    if get_user_by_email(email):
        return RedirectResponse("/register?error=Email уже используется", 303)
    if get_user_by_nickname(nickname):
        return RedirectResponse("/register?error=Никнейм уже занят", 303)
    if not is_valid_email(email):
        return RedirectResponse("/register?error=Неверный формат почты", 303)
    ...
    return RedirectResponse("/login?success=Регистрация успешна! Подтвердите email по ссылке в письме", 303)
...
async def verify_email(request: Request, code: str):
    ...
    return RedirectResponse("/login?error=Неверный или просроченный код", 303)

@app.get("/forgot_password", response_class=HTMLResponse)
async def forgot_password_page(request: Request, error: str = None):
    user = get_user_from_session(request)
    err = f'<div class="error">{error}</div>' if error else ''
    content = f'<section style="padding:50px 0;"><div class="container"><div style="max-width:400px;margin:0 auto;background:var(--card-bg);padding:30px;border-radius:20px;"><h2>Восстановление пароля</h2>{err}<form action="/forgot_password" method="post"><div class="form-group"><label>Email</label><input type="email" name="email" required></div><button type="submit" class="btn">Отправить</button></form><p><a href="/login">Вспомнили пароль?</a></p></div></div></section>'
    return HTMLResponse(render_html(content, user=user))
...
async def forgot_password_post(email: str = Form(...)):
    ...
    return RedirectResponse("/login?success=Если email зарегистрирован, вы получите письмо", 303)
...
async def reset_password_post(code: str = Form(...), new_password: str = Form(...), confirm_password: str = Form(...)):
    ...
    return RedirectResponse("/login?success=Пароль изменён! Теперь вы можете войти", 303)
...
async def profile(request: Request, error: str = None, success: str = None):
    ...
    for p in history_props[:10]:
        history_html += f'<div class="property-card" style="margin-bottom:10px;"><a href="/buy?search={p["title"]}" style="text-decoration:none;"><h4>{p["title"]} - {p["price"]} ₽</h4></a></div>'
    if not history_html:
    ...
    <section style="padding:50px 0;"><div class="container">
        <div style="background:var(--card-bg);border-radius:24px;padding:40px;">
            ...
            <div style="margin:30px 0;padding:20px;background:var(--bg);border-radius:16px;">
                ...
                <p><strong>📧 Email:</strong> {email} ({"✅ Подтверждён" if email_verified else "❌ Не подтверждён"})</p>
            </div>
            <form action="/update_nickname" method="post"><div class="form-group"><label>Изменить никнейм</label><div style="display:flex;gap:10px;"><input type="text" name="nickname" value="{nickname}" style="flex:1;" required><button type="submit" class="btn">Обновить</button></div></div></form>
            <form action="/update_email" method="post"><div class="form-group"><label>Изменить email</label><div style="display:flex;gap:10px;"><input type="email" name="email" value="{email}" style="flex:1;"><button type="submit" class="btn">Сохранить</button></div></div></form>
            {'' if email_verified else '<div style="margin:15px 0;"><a href="/send_verification" class="btn">📧 Отправить повторно</a></div>'}
            <h2 style="margin:30px 0 20px;">🔔 Уведомления</h2><div>{notif_html}</div>
            <h2 style="margin:30px 0 20px;">🕐 История просмотров</h2><div>{history_html}</div>
        </div>
    </div></section>'''
    theme = user.get("theme", "light")
    return HTMLResponse(render_html(content, user=user, theme=theme))
...
async def send_verification_route(request: Request):
    user = get_user_from_session(request)
    if not user:
        return RedirectResponse("/login", 303)
    user_data = get_user_by_username(user["username"])
    ...
    if not user_data.get("email"):
        return RedirectResponse("/profile?error=Сначала укажите email", 303)
...
async def update_nickname_route(request: Request, nickname: str = Form(...)):
    user = get_user_from_session(request)
    if not user:
        return RedirectResponse("/login", 303)
...
async def update_email_route(request: Request, email: str = Form(...)):
    user = get_user_from_session(request)
    if not user:
        return RedirectResponse("/login", 303)
    if email and not is_valid_email(email):
        return RedirectResponse("/profile?error=Неверный формат почты", 303)
    if get_user_by_email(email) and get_user_by_email(email)["username"] != user["username"]:
        return RedirectResponse("/profile?error=Email уже используется", 303)
    ...
    return RedirectResponse("/profile?success=Email обновлён. Подтвердите его по ссылке в письме", 303)
